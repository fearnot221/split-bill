...
單一共享帳本的記帳分帳工具：帳本本身用來記錄共同帳目，把朋友加進成員清單後，記錄誰付了錢、怎麼分攤，自動算清誰欠誰。支援手機、平板、桌面（響應式設計＋深色模式）。

**線上版**：<https://bill.fearnot.tw>（自架伺服器，所有裝置共用同一份資料）

## 功能

- **單一共享帳本**：帳本直接記錄共同帳目，打開就能用，免註冊、免建立群組或額外建立公用帳戶
...
- **成員轉帳**：記帳視窗選「轉帳」可記錄成員間的還款與代墊結算，只調整雙方結餘，不計入消費統計
- **即時結餘**：每位成員的應收/應付一目了然
...
- **成員管理**：新增、改名（帳目跟著更新）、刪除；有帳務紀錄的成員不可刪除，帳本至少保留一位成員
