...
function calculateLedger(members, expenses, wallets = []) {
  ...
  const walletLedgers = new Map();
  for (const wallet of wallets) {
    const id = readId(wallet, 'Wallet');
    if (walletLedgers.has(id)) throw new RangeError(`Duplicate wallet: ${id}`);
    walletLedgers.set(id, {
      balanceCents: 0,
      positions: new Map([...balances.keys()].map((memberId) => [memberId, 0])),
  ...
  for (const expense of expenses) {
    ...
    const payerId = expense.payer_id ?? expense.payerId ?? null;
    const payerWalletId = expense.payer_wallet_id ?? expense.payerWalletId ?? null;
    if ((payerId === null) === (payerWalletId === null)) {
      throw new RangeError('Entry must have exactly one member or wallet source');
    }
    if (payerId !== null) requireMember(balances, payerId, 'Payer');
    if (payerWalletId !== null) requireWallet(walletLedgers, payerWalletId, 'Payer wallet');
    ...
    const explicitTargetMemberId = expense.transfer_to_member_id
      ?? expense.transferToMemberId
      ?? null;
    const explicitTargetWalletId = expense.transfer_to_wallet_id
      ?? expense.transferToWalletId
      ?? null;
    if (!isTransfer && (explicitTargetMemberId !== null || explicitTargetWalletId !== null)) {
      throw new RangeError('Only transfers can have a target account');
    }
    if (isTransfer) {
      let targetMemberId = explicitTargetMemberId;
      let targetWalletId = explicitTargetWalletId;
      const hasExplicitTarget = targetMemberId !== null || targetWalletId !== null;
      if (hasExplicitTarget && expense.splits !== undefined
        && (!Array.isArray(expense.splits) || expense.splits.length > 0)) {
        throw new RangeError('Transfer cannot contain expense splits');
      }
      if (!hasExplicitTarget) {
        if (!Array.isArray(expense.splits) || expense.splits.length !== 1) {
          throw new RangeError('Transfer must have exactly one target');
        }
        const legacyTarget = expense.splits[0];
        targetMemberId = legacyTarget.member_id ?? legacyTarget.memberId ?? null;
        if (moneyToCents(legacyTarget.amount) !== amountCents) {
          throw new RangeError('Transfer target amount must equal the entry amount');
        }
      }
      if ((targetMemberId === null) === (targetWalletId === null)) {
        throw new RangeError('Transfer must have exactly one member or wallet target');
      }
      if (targetMemberId !== null) requireMember(balances, targetMemberId, 'Transfer target');
      if (targetWalletId !== null) requireWallet(walletLedgers, targetWalletId, 'Transfer target wallet');
      if ((payerId !== null && payerId === targetMemberId)
        || (payerWalletId !== null && payerWalletId === targetWalletId)) {
        throw new RangeError('Transfer source and target must be different');
      }
      if (payerWalletId !== null && targetWalletId !== null) {
        throw new RangeError('Wallet-to-wallet transfers are not supported');
      }

      if (payerId !== null && targetMemberId !== null) {
        balances.set(payerId, safeAdd(balances.get(payerId), amountCents, 'Member balance'));
        balances.set(
          targetMemberId,
          safeAdd(balances.get(targetMemberId), -amountCents, 'Member balance')
        );
      } else if (payerId !== null) {
        const wallet = walletLedgers.get(targetWalletId);
        wallet.balanceCents = safeAdd(wallet.balanceCents, amountCents, 'Wallet balance');
        wallet.positions.set(
          payerId,
          safeAdd(wallet.positions.get(payerId), amountCents, 'Wallet position')
        );
      } else {
        const wallet = walletLedgers.get(payerWalletId);
        wallet.balanceCents = safeAdd(wallet.balanceCents, -amountCents, 'Wallet balance');
        wallet.positions.set(
          targetMemberId,
          safeAdd(wallet.positions.get(targetMemberId), -amountCents, 'Wallet position')
        );
      }
      continue;
    }
    ...
    if (!Array.isArray(expense.splits)) {
      throw new RangeError('Expense splits must be an array');
    }
    if (expense.splits.length === 0) {
      if (payerWalletId === null || isIncome) {
        throw new RangeError('Expense must have at least one split');
      }
      const wallet = walletLedgers.get(payerWalletId);
      wallet.balanceCents = safeAdd(wallet.balanceCents, -amountCents, 'Wallet balance');
      wallet.sharedBalanceCents = safeAdd(
        wallet.sharedBalanceCents,
        -amountCents,
        'Wallet shared balance'
      );
    ...
    if (payerId !== null) {
      ...
      for (const [memberId, splitCents] of parsedSplits) {
        wallet.positions.set(
          memberId,
          safeAdd(wallet.positions.get(memberId), -direction * splitCents, 'Wallet position')
        );
      }
    }
  ...
  for (const [walletId, wallet] of walletLedgers) {
    const positionTotal = [...wallet.positions.values()].reduce(
      (sum, position) => safeAdd(sum, position, 'Wallet position total'),
      0
    );
    if (safeAdd(positionTotal, wallet.sharedBalanceCents, 'Wallet position total')
      !== wallet.balanceCents) {
      throw new Error(`Wallet ${walletId} positions do not match its balance`);
    }
  }

  return {
    balancesCents: Object.fromEntries(balances),
    walletLedgers: Object.fromEntries([...walletLedgers].map(([id, wallet]) => [id, {
      balanceCents: wallet.balanceCents,
      positionsCents: Object.fromEntries(wallet.positions),
      sharedBalanceCents: wallet.sharedBalanceCents,
    }])),
    totalExpenseCents,
    totalIncomeCents,
  };
...
function requireWallet(walletLedgers, id, label) {
  if (!walletLedgers.has(id)) throw new RangeError(`${label} is not a ledger wallet`);
}
