    ...
    for (const member of mentioned) {
      ...
      const chineseMatch = parsedAmount === null
        ? source.match(new RegExp(
          `${escapeRegExp(member.name)}\\s*[:：]?\\s*([零〇一二兩三四五六七八九十百千萬]+)(?:元|塊)?`
        ))
        : null;
...
function findAmount(text) {
  ...
  const scaled = /(?<![\d.,])(?:(?:NT\$|TWD|\$)\s*)?([\d,]+(?:\.\d{1,2})?)\s*[kK](?![A-Za-z])/i.exec(scrubbed);
  if (scaled) {
  ...
  const patterns = [
    /(?:總共|共計|合計|總額|金額|共)\s*(?:(?:NT\$|TWD|\$)\s*)?([\d,]+(?:\.\d{1,2})?)(?![\d.kK])/i,
    /(?:花了?|消費|支出)\s*(?:(?:NT\$|TWD|\$)\s*)?([\d,]+(?:\.\d{1,2})?)(?![\d.kK])/i,
    /(?:NT\$|TWD|\$)\s*([\d,]+(?:\.\d{1,2})?)(?![\d.kK])/i,
    /(?<![\d.,])([\d,]+(?:\.\d{1,2})?)(?![\d.kK])\s*(?:元|塊)/,
  ];
  ...
  const chinesePatterns = [
    new RegExp(`(?:總共|共計|合計|總額|金額|共)\\s*([${chineseDigits}]+)(?![${chineseDigits}人份])\\s*(?:元|塊)?`),
    new RegExp(`([${chineseDigits}]+)\\s*(?:元|塊)`),
  ];
...
function cleanDescription(value, kind, category, members = []) {
  ...
  for (const member of members) {
    ...
    cleaned = cleaned
      ...
      .replace(new RegExp(`${name}\\s*[:：]?\\s*[零〇一二兩三四五六七八九十百千萬]+(?:元|塊)?`, 'g'), '')
