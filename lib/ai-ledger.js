...
const AI_DRAFT_SCHEMA = {
  ...
  required: [
    ...
    'payerName', 'payerAccountType', 'participantNames', 'splitMode', 'customSplits',
    'transferToName', 'transferToAccountType', 'note', 'confidence', 'warnings',
  ],
  properties: {
    ...
    payerAccountType: {
      type: ['string', 'null'],
      enum: ['member', 'public_wallet', null],
    },
...
function buildInstructions(context, today) {
  ...
  const contextWallet = normalizedWallet(context);
  const wallet = contextWallet ? { name: contextWallet.name, type: 'public_wallet' } : null;
  ...
  const explicitParticipantNames = context.members
    .filter((member) => explicitParticipantIds.has(member.id))
  ...
  return [
    ...
    wallet ? `帳本公帳錢包：${JSON.stringify(wallet)}` : null,
    wallet
      ? `public_wallet 的同義名稱為：${JSON.stringify(walletAliasNames(wallet))}；這些名稱都代表同一個公帳錢包。`
      : null,
    `可用分類（必須完全取自此清單）：${JSON.stringify(categories)}`,
    '規則：amount 是單筆總額；participantNames 只能是帳本成員，表示實際承擔支出或分配收入的人，包含付款人本人時也要列出。',
    explicitParticipantNames.length
      ...
      : null,
    'payerAccountType 與 transferToAccountType 必須明確標示 member、public_wallet 或 null，並與對應名稱一致。',
    wallet && members.some((member) => member.name === wallet.name)
      ? `成員中也有人名為「${wallet.name}」：單獨提到此名稱時一律視為 member；只有明確寫「帳本錢包」、「公帳錢包」或「錢包」才是 public_wallet。`
      : null,
    '「大家」、「全員」代表所有帳本成員，不包含 public_wallet。不分攤時 splitMode=none；成員付款只列 payerName，公帳錢包支付共同支出時 participantNames 留空。',
    '自訂金額或百分比時 splitMode=custom；百分比要依 amount 換算為實際金額，customSplits 必須列出每人金額且總和等於 amount。其他分帳預設 splitMode=equal。',
    '公帳付款或收款時 payerAccountType=public_wallet；participantNames 仍只能列成員。公帳支付可用 splitMode=none 表示不歸屬任何成員的共同支出；公帳收入仍至少要有一位分配成員。',
    '轉帳時 kind=transfer、transferToName 是成員或 public_wallet，並設定相符的 transferToAccountType；participantNames 留空，category 留 null。「存入公帳」是成員轉給 public_wallet。',
...
function receiptFieldEvidence(raw, normalizedDraft) {
  ...
  const transferProvided = normalizedDraft.kind !== 'transfer'
    || (!!cleanString(raw?.transferToName, 40)
      ...
      && hasDraftTransferTarget(normalizedDraft));
  return {
...
function mergeReceiptResults(primary, detailed, textualEvidence = {}) {
  ...
  if (sameKind && !preferredFieldEvidence?.payerProvided
    && alternateFieldEvidence?.payerReliable && hasDraftPayer(alternate)) {
    draft.payerSource = alternate.payerSource;
    draft.payerId = alternate.payerId;
    draft.payerWalletId = alternate.payerWalletId;
  ...
  if (sameKind && draft.kind === 'transfer' && !preferredFieldEvidence?.transferProvided
    && alternateFieldEvidence?.transferReliable && hasDraftTransferTarget(alternate)) {
    draft.transferToSource = alternate.transferToSource;
    draft.transferToId = alternate.transferToId;
    draft.transferToWalletId = alternate.transferToWalletId;
  ...
  if (resolvedFallbackFields.has('payer')
    || (hasDraftPayer(draft) && fieldEvidence.some((evidence) => evidence?.payerReliable))) {
    draft.warnings = draft.warnings.filter((warning) =>
  ...
  if (draft.kind === 'transfer' && hasDraftTransferTarget(draft)
  ...
  draft.ready = !!draft.isLedgerEntry
    ...
    && hasDraftPayer(draft)
    && (draft.kind === 'transfer'
      ? hasDraftTransferTarget(draft)
      : Array.isArray(draft.participantIds) && draft.participantIds.length > 0
        || (draft.kind === 'expense'
          && draft.payerSource === 'wallet'
...
function hasDraftPayer(draft) {
  return draft?.payerSource === 'wallet' ? !!draft.payerWalletId : !!draft?.payerId;
}

function hasDraftTransferTarget(draft) {
  return draft?.transferToSource === 'wallet'
    ? !!draft.transferToWalletId
    : !!draft?.transferToId;
}

function normalizedWallet(context) {
  if (!context?.wallet || typeof context.wallet !== 'object') return null;
  const id = cleanString(context.wallet.id, 100);
  const name = cleanString(context.wallet.name, 40);
  return id && name ? { id, name } : null;
}

function walletAliasNames(wallet) {
  if (!wallet) return [];
  return [...new Set([
    wallet.name, '公帳', '公帳錢包', '帳本錢包', '錢包',
  ].map((name) => cleanString(name, 40)).filter(Boolean))];
}

function isWalletAlias(value, wallet) {
  const name = cleanString(value, 40).toLocaleLowerCase();
  if (!name || !wallet) return false;
  return walletAliasNames(wallet).some((alias) => alias.toLocaleLowerCase() === name);
}
...
function normalizeDraft(raw, context) {
  ...
  const wallet = normalizedWallet(context);
  const explicitParticipantIds = new Set(
    Array.isArray(context.explicitParticipantIds) ? context.explicitParticipantIds : []
  );
  const explicitParticipants = members.filter((member) => explicitParticipantIds.has(member.id));
  ...
  const accountForMember = (member) => member
    ? { source: 'member', id: member.id, name: member.name }
    : null;
  const resolveAccount = (value, accountType) => {
    const name = cleanString(value, 40);
    const requestedSource = accountType === 'public_wallet'
      ? 'wallet'
      : accountType === 'member' ? 'member' : null;
    if (!name) return null;
    if (requestedSource === 'wallet') {
      return isWalletAlias(name, wallet)
        ? { source: 'wallet', id: wallet.id, name: wallet.name }
        : null;
    }
    if (['我', '自己', '本人'].includes(name) && defaultMember) {
      return requestedSource === 'wallet' ? null : accountForMember(defaultMember);
    }
    const member = findMember(name);
    if (member) return accountForMember(member);
    if (requestedSource === 'member') return null;
    if (isWalletAlias(name, wallet)) {
      return { source: 'wallet', id: wallet.id, name: wallet.name };
    }
    return null;
  };
  const sameAccount = (left, right) => !!left && !!right
    && left.source === right.source && left.id === right.id;

  const isLedgerEntry = raw?.isLedgerEntry !== false;
  const kind = ['expense', 'income', 'transfer'].includes(raw?.kind) ? raw.kind : 'expense';
  const rawPayerName = cleanString(raw?.payerName, 40);
  const providedPayer = resolveAccount(rawPayerName, raw?.payerAccountType);
  const payer = rawPayerName ? providedPayer : accountForMember(defaultMember);
  if (rawPayerName && !providedPayer) {
    warnings.push(`找不到付款／收款人「${cleanString(raw.payerName, 20)}」`);
  } else if (!rawPayerName && payer) {
    warnings.push(`尚未辨識付款／收款人，已暫設為「${payer.name}」`);
  }
  ...
  for (const name of Array.isArray(raw?.participantNames) ? raw.participantNames : []) {
    const member = findMember(name);
    if (!member && isWalletAlias(name, wallet)) {
      if (!explicitParticipants.length) warnings.push(`「${wallet.name}」不能作為一般分攤成員`);
      continue;
    }
    if (!member) {
  ...
  if (splitMode === 'custom') {
    for (const split of Array.isArray(raw?.customSplits) ? raw.customSplits : []) {
      ...
      if (!member && isWalletAlias(split?.memberName, wallet)) continue;
      if (!member) continue;
      try {
  ...
  let transferTo = null;
  if (kind === 'transfer') {
    transferTo = resolveAccount(raw?.transferToName, raw?.transferToAccountType);
    if (!transferTo) warnings.push('尚未辨識轉帳收款人');
    else if (sameAccount(transferTo, payer)) {
    ...
    participantMap.clear();
    if (payer?.source === 'member') {
      const member = members.find((candidate) => candidate.id === payer.id);
      if (member) participantMap.set(member.id, member);
    }
  } else if (participantMap.size === 0 && payer?.source === 'member') {
    const member = members.find((candidate) => candidate.id === payer.id);
    if (member) participantMap.set(member.id, member);
    splitMode = 'none';
    warnings.push('尚未辨識分攤成員，暫設為付款／收款人自行承擔');
  }
  if (kind === 'income' && payer?.source === 'wallet' && participantMap.size === 0) {
    warnings.push('帳本錢包收入需指定至少一位分配成員');
  }
  ...
  const ready = isLedgerEntry
    ...
    && (kind === 'transfer'
      ...
      : participantMap.size > 0
        || (kind === 'expense' && payer.source === 'wallet' && splitMode === 'none'));
  ...
  return {
    ...
    category,
    expenseDate,
    payerSource: payer?.source || null,
    payerId: payer?.source === 'member' ? payer.id : null,
    payerWalletId: payer?.source === 'wallet' ? payer.id : null,
    payerName: payer?.name || null,
    participantIds: [...participantMap.keys()],
    participantNames: [...participantMap.values()].map((member) => member.name),
    splitMode,
    customSplits,
    transferToSource: transferTo?.source || null,
    transferToId: transferTo?.source === 'member' ? transferTo.id : null,
    transferToWalletId: transferTo?.source === 'wallet' ? transferTo.id : null,
...
function localParse(text, context) {
  ...
  const defaultMember = members.find((member) => member.id === context.defaultMemberId)
    || members[0];
  const memberAccount = (member) => member
    ? { source: 'member', id: member.id, name: member.name }
    : null;
  const publicWalletAccount = wallet
    ? { source: 'wallet', id: wallet.id, name: wallet.name }
    : null;
  const aliasToAccount = new Map(members.map((member) => [member.name, memberAccount(member)]));
  if (defaultMember) {
    aliasToAccount.set('我', memberAccount(defaultMember));
    aliasToAccount.set('自己', memberAccount(defaultMember));
    aliasToAccount.set('本人', memberAccount(defaultMember));
  }
  for (const alias of walletAliasNames(wallet)) {
    if (!['我', '自己', '本人'].includes(alias) && !aliasToAccount.has(alias)) {
      aliasToAccount.set(alias, { ...publicWalletAccount, name: alias });
    }
  }
  const accountForAlias = (value) => {
    const normalized = cleanString(value, 40).toLocaleLowerCase();
    if (!normalized) return null;
    for (const [alias, account] of aliasToAccount) {
      if (alias.toLocaleLowerCase() === normalized) return account;
    }
    return null;
  };
  const accountAliasEntries = [...aliasToAccount.entries()]
    .sort(([left], [right]) => right.length - left.length);
  const accountAliases = accountAliasEntries
    .map(([alias]) => alias)
    .sort((a, b) => b.length - a.length)
    .map(escapeRegExp)
    .join('|');
  const directTransferMatch = accountAliases
    ? new RegExp(
      `(${accountAliases})\\s*(?:付給|給)\\s*(${accountAliases})(?=\\s*(?:(?:NT\\$|TWD|NTD|新台幣|台幣|[$＄])?\\s*(?:\\d|[零〇一二兩三四五六七八九十百千萬])))`,
      'i'
    ).exec(source)
    : null;
  const directTransfer = directTransferMatch
    ? {
        from: accountForAlias(directTransferMatch[1]),
        to: accountForAlias(directTransferMatch[2]),
      }
    : null;
  if (directTransfer && directTransfer.from?.source === directTransfer.to?.source
    && directTransfer.from?.id === directTransfer.to?.id) directTransfer.to = null;
  const accountTransfer = accountAliasEntries.some(([alias]) => new RegExp(
    `(?:轉|匯|還)\\s*${escapeRegExp(alias)}(?=\\s*(?:\\d|NT\\$|TWD|\\$|元|塊|[，,、。]|$))`,
    'i'
  ).test(source));
  const walletDeposit = walletAliasNames(wallet).some((alias) => new RegExp(
    `(?:存入|存到|儲值(?:到|進)?)\\s*${escapeRegExp(alias)}(?=\\s*(?:\\d|NT\\$|TWD|\\$|元|塊|[，,、。]|$))`,
    'i'
  ).test(source));
  const kind = directTransfer?.to || accountTransfer || walletDeposit
  ...
  let payer = directTransfer?.from || memberAccount(defaultMember);
  const payerMatch = accountAliasEntries.find(([alias]) => {
    const escaped = escapeRegExp(alias);
    const payerAfterName = kind === 'income'
      ? '(?:收到|收了|收(?:款)?|入帳)'
      : kind === 'transfer'
        ? '(?:轉(?:帳)?|匯(?:款|給)?|還(?:款|給)?|存(?:入|到)?|儲值)'
        : '(?:付(?:款)?|支付|墊(?:付)?|刷卡|請客|出(?:的|錢)?)';
    const payerLabel = kind === 'income' ? '收款人' : '付款人';
    return new RegExp(`${escaped}\\s*(?:先\\s*)?${payerAfterName}`).test(source)
      || new RegExp(`${payerLabel}\\s*(?:是|為|[:：])?\\s*${escaped}`).test(source)
      || new RegExp(`由\\s*${escaped}\\s*${payerAfterName}`).test(source);
  });
  if (payerMatch) payer = payerMatch[1];

  let transferTo = null;
  if (kind === 'transfer') {
    transferTo = directTransfer?.to || accountAliasEntries.find(([alias, account]) => {
      if (account?.source === payer?.source && account?.id === payer?.id) return false;
      const escaped = escapeRegExp(alias);
      return new RegExp(
        `(?:給|匯給|還給|轉入|轉到|到|存入|存到|儲值(?:到|進)?).{0,3}${escaped}|(?:轉|匯|還)\\s*${escaped}|${escaped}.{0,3}(?:收|收款)`
      ).test(source);
    })?.[1] || memberAccount(mentioned.find((member) =>
      payer?.source !== 'member' || member.id !== payer.id
    )) || null;
  }
  ...
  if (!labeledParticipants.length && equalSplitCue && participants.length === 1
    && payer?.source === 'member' && payer.id === defaultMember?.id
    && participants[0].id !== payer.id) {
    participants = [defaultMember, ...participants];
  }
  ...
  if (kind !== 'transfer' && !noSplit && !hasParticipantCue) {
    ...
    const payerMember = payer?.source === 'member'
      ? members.find((member) => member.id === payer.id)
      : null;
    participants = payerMember ? [payerMember] : [];
  ...
  const descriptionAccounts = [
    ...members,
    ...walletAliasNames(wallet).map((name) => ({ name })),
  ];
  const labeledDescription = extractLabeledDescription(source, kind, category, descriptionAccounts);
  let description = labeledDescription || (amountMatch
    ? cleanDescription(source.slice(0, amountMatch.index), kind, category, descriptionAccounts)
    : cleanDescription(source, kind, category, descriptionAccounts));
  if (!labeledDescription && amountMatch && isGenericDescription(description, kind, category)) {
    const trailing = extractTrailingDescription(
      source.slice(amountMatch.end), kind, category, descriptionAccounts
    );
  ...
  return {
    ...
    expenseDate,
    payerName: payer?.name || null,
    payerAccountType: payer?.source === 'wallet' ? 'public_wallet' : payer ? 'member' : null,
    ...
    customSplits,
    transferToName: transferTo?.name || null,
    transferToAccountType: transferTo?.source === 'wallet'
      ? 'public_wallet'
      : transferTo ? 'member' : null,
    note: note || null,
...
function extractTrailingDescription(value, kind, category, members) {
  ...
  const memberAction = names
    ? `(?:由\\s*)?(?:${names})\\s*(?:先\\s*)?(?:付(?:款)?|支付|墊(?:付)?|刷卡|請客|出(?:的|錢)?|收(?:到|了|款)?|入帳|轉|匯|還|存(?:入|到)?|儲值|跟|和|與|、)`
    : '(?!)';
...
function cleanDescription(value, kind, category, members = []) {
  ...
  if (kind === 'transfer') {
    for (const member of members) {
      const name = escapeRegExp(member.name);
      cleaned = cleaned
        .replace(new RegExp(`(?:給|匯給|還給|轉入|轉到|到|轉|匯|還|存入|存到|儲值(?:到|進)?)\\s*${name}`, 'g'), '')
        .replace(new RegExp(`${name}\\s*(?:轉帳|匯款|還款|轉|匯|還|存入|存到|儲值)`, 'g'), '')
        .replace(new RegExp(`(^|[\\s，,、:：])${name}(?=$|[\\s，,、:：])`, 'g'), '$1');
    }
    cleaned = cleaned.replace(/(轉帳|轉給|轉入|轉到|匯款|匯給|還款|還給|存入|存到|儲值|(?:轉|匯|還|到)(?=$|[\s，,、:：]))/g, '');
  }
