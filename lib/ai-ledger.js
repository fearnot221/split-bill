...
async function analyzeWithOpenAI({
  ...
  if (!receiptDataUrl || !needsHighDetailReceiptRetry(primary)) return primary;

  try {
    const detailed = await analyze('high');
    const upgraded = mergeReceiptResults(primary, detailed);
    return {
      ...upgraded,
...
function parseOpenAIResponse(response, normalizationContext) {
  ...
  return {
    draft: normalizeDraft(raw, normalizationContext),
    usage,
...
function rawReceiptNeedsHighDetail(raw) {
  ...
  const warnings = normalizeWarnings(raw?.warnings).join(' ');
  return raw?.isLedgerEntry === false
    ...
    || raw.confidence < 0.6
    || /(未辨識|無法讀取|模糊|看不清|不清楚)/.test(warnings);
}

function needsHighDetailReceiptRetry(result) {
  return result?.extractionNeedsHighDetail === true;
}

function receiptDraftQuality(result) {
  const draft = result?.draft || {};
  let score = 0;
  if (draft.isLedgerEntry) score += 3;
  if (draft.amount !== null && draft.amount !== undefined) score += 5;
  if (draft.description) score += 4;
  if (draft.payerId) score += 3;
  if (draft.expenseDate) score += 1;
  if (draft.kind !== 'transfer' || draft.transferToId) score += 2;
  if (draft.ready) score += 8;
  score += Math.max(0, Math.min(1, Number(draft.confidence) || 0)) * 2;
  if (result?.extractionNeedsHighDetail) score -= 2;
  return score;
}

function mergeReceiptResults(primary, detailed) {
  const preferDetailed = receiptDraftQuality(detailed) >= receiptDraftQuality(primary);
  ...
  if (!draft.isLedgerEntry && alternate.isLedgerEntry) {
    draft.isLedgerEntry = true;
    backfilled = true;
  }
  if (!draft.description && alternate.description) {
    draft.description = alternate.description;
    backfilled = true;
  }
  if ((draft.amount === null || draft.amount === undefined)
    && alternate.amount !== null && alternate.amount !== undefined) {
    draft.amount = alternate.amount;
    backfilled = true;
  }
  if (!draft.payerId && alternate.payerId) {
    draft.payerId = alternate.payerId;
    draft.payerName = alternate.payerName;
    backfilled = true;
  }
  if (draft.kind === 'transfer' && !draft.transferToId && alternate.transferToId) {
  ...
  if (draft.kind !== 'transfer' && draft.splitMode !== 'none'
    && draft.participantIds.length === 0 && alternate.participantIds?.length) {
  ...
  draft.warnings = [...new Set(preferred.warnings || [])];
  if (backfilled) {
    ...
    draft.warnings = draft.warnings.filter((warning) => !(
      (draft.amount !== null && /尚未辨識金額/.test(warning))
      || (draft.description && /尚未辨識項目說明/.test(warning))
    ));
    draft.warnings.push('細節辨識結果不一致，已保留較完整欄位，請逐項確認');
  }
  draft.warnings = [...new Set(draft.warnings)].slice(0, 8);
...
function localParse(text, context) {
  ...
  if (isoDate) {
  ...
  else if (/大後天/.test(source)) expenseDate = addDays(context.today, 3);
  else if (/後天/.test(source)) expenseDate = addDays(context.today, 2);
  else if (/明天/.test(source)) expenseDate = addDays(context.today, 1);
  else {
    ...
    if (relativeWeekday) {
      ...
      expenseDate = addDays(context.today, mondayOffset + weekOffset + weekdayOffset);
    } else {
      ...
      if (chineseDate || yearlessChineseDate || shortDate) {
        const compactDate = yearlessChineseDate || shortDate;
        const parts = chineseDate || [null, context.today.slice(0, 4), compactDate[1], compactDate[2]];
