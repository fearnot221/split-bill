...
function localParse(text, context) {
  ...
  const kind = /(轉帳|轉給|轉入|轉到|轉(?=\s*\d)|匯款|匯給|還款|還給)/.test(source)
    ? 'transfer'
  ...
  if (mentionsEveryone(source, people.length)) {
  ...
  if (kind === 'transfer') {
    transferTo = context.members.find((member) => {
      const escaped = escapeRegExp(member.name);
      return new RegExp(`(給|匯給|還給|到|轉入).{0,3}${escaped}|${escaped}.{0,3}(收|收款)`).test(source);
...
function findAmount(text) {
  ...
  const patterns = [
    /(?:總共|共計|合計|總額|金額|共)\s*(?:(?:NT\$|TWD|\$)\s*)?([\d,]+(?:\.\d{1,2})?)/i,
    /(?:花了?|消費|支出)\s*(?:(?:NT\$|TWD|\$)\s*)?([\d,]+(?:\.\d{1,2})?)/i,
    /(?:NT\$|TWD|\$)\s*([\d,]+(?:\.\d{1,2})?)/i,
    /([\d,]+(?:\.\d{1,2})?)\s*(?:元|塊)/,
  ];
...
function cleanDescription(value, kind, category, members = []) {
  let cleaned = cleanString(value, 200)
    ...
    .replace(/(大前天|前天|昨天|今天|請幫我|記一筆|記帳)/g, '')
