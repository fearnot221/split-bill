...
function buildInstructions(context, today) {
  ...
  return [
    ...
    wallet && members.some((member) => member.name === wallet.name)
      ...
      : null,
    '「大家」、「全員」代表所有帳本成員，不包含 public_wallet。不分攤時 splitMode=none 並只列 payerName。',
    '自訂金額或百分比時 splitMode=custom；百分比要依 amount 換算為實際金額，customSplits 必須列出每人金額且總和等於 amount。其他分帳預設 splitMode=equal。',
    '公帳付款或收款時 payerAccountType=public_wallet；participantNames 仍只能列成員，而且至少要有一位。',
...
function normalizeDraft(raw, context) {
  ...
  if (kind !== 'transfer' && payer?.source === 'wallet' && participantMap.size === 0) {
    warnings.push('帳本錢包收支需指定至少一位分攤成員');
  }
  ...
  const ready = isLedgerEntry
    ...
    && !!description
    && !!payer
    && (kind === 'transfer' ? !!transferTo : participantMap.size > 0);
