...
function buildInstructions(context, today) {
  ...
  return [
    ...
    '自訂金額時 splitMode=custom，customSplits 必須列出每人金額，總和應等於 amount。其他分帳預設 splitMode=equal。',
...
async function analyzeWithOpenAI({ client, model, text, receiptDataUrl, context, today, safetyIdentifier }) {
  const response = await client.responses.create(buildOpenAIRequest({
    model, text, receiptDataUrl, context, today, safetyIdentifier,
  }));
  const refusal = response.output
    ?.flatMap((item) => item.content || [])
    .find((item) => item.type === 'refusal');
  if (refusal) throw new Error(refusal.refusal || '無法分析這筆內容');
  if (!response.output_text) throw new Error('AI 沒有回傳可用的帳目資料');
  let raw;
  try {
    raw = JSON.parse(response.output_text);
  } catch {
    throw new Error('AI 回傳的帳目格式無法讀取');
  }
  return normalizeDraft(raw, { ...context, today, sourceText: text });
}
...
function localParse(text, context) {
  ...
  const kind = /(轉帳|匯款|匯給|還款|還給)/.test(source)
    ? 'transfer'
    : /(收入|薪水|退款|收款|入帳)/.test(source) ? 'income' : 'expense';
  ...
  for (const member of context.members) {
    ...
    if (new RegExp(`${escaped}\\s*(?:先\\s*)?(付|墊|刷卡|請客)`).test(source)) payer = member;
  }
  ...
  if (kind === 'transfer') {
    transferTo = context.members.find((member) => {
      const escaped = escapeRegExp(member.name);
      return new RegExp(`(給|匯給|還給).{0,3}${escaped}|${escaped}.{0,3}(收|收款)`).test(source);
    }) || mentioned.find((member) => member.id !== payer?.id) || null;
  }

  const categoryRules = [
    ['餐飲', /(餐|吃|飯|咖啡|飲料|夜市|早餐|午餐|晚餐)/],
    ['交通', /(車|捷運|高鐵|火車|計程車|油錢|停車|機票)/],
    ['住宿', /(住宿|旅館|飯店|民宿|房費)/],
    ['購物', /(購物|買|商店|超市)/],
    ['娛樂', /(娛樂|電影|門票|唱歌|遊戲)/],
  ];
  const categoryNames = new Set(context.categories.map((category) => category.name));
  const category = categoryRules.find(([name, pattern]) => categoryNames.has(name) && pattern.test(source))?.[0]
  ...
  for (const member of mentioned) {
    const match = source.match(new RegExp(`${escapeRegExp(member.name)}\\s*[:：]?\\s*(\\d[\\d,]*(?:\\.\\d{1,2})?)`));
  ...
  const description = amountMatch
    ? cleanDescription(source.slice(0, amountMatch.index), kind, category)
    : cleanDescription(source, kind, category);
  ...
  return {
    ...
    expenseDate,
    payerName: payer?.name || null,
    participantNames: kind === 'transfer' ? [] : mentioned.map((member) => member.name),
    splitMode: noSplit ? 'none' : customSplits.length ? 'custom' : mentioned.length ? 'equal' : 'none',
    customSplits,
    transferToName: transferTo?.name || null,
    note: null,
    confidence: amountMatch ? 0.58 : 0.25,
    warnings: context.hasReceipt
...
function findAmount(text) {
  ...
  for (const pattern of patterns) {
    ...
    if (match) return { amount: Number(match[1].replaceAll(',', '')), index: match.index };
  }
  const matches = [...text.matchAll(/\d[\d,]*(?:\.\d{1,2})?/g)]
    .filter((match) => !/^\d{4}$/.test(match[0]) && !/\d{4}[-/]$/.test(text.slice(Math.max(0, match.index - 5), match.index + match[0].length + 1)));
  const match = matches.find((candidate) => Number(candidate[0].replaceAll(',', '')) > 0);
  return match ? { amount: Number(match[0].replaceAll(',', '')), index: match.index } : null;
}

function cleanDescription(value, kind, category) {
  const cleaned = cleanString(value, 50)
    .replace(/(昨天|今天|前天|請幫我|記一筆|記帳)/g, '')
    .trim();
